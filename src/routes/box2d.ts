...
export const box2D: typeof Box2D & EmscriptenModule = await Box2DFactory({
   ...
   */
  locateFile: (url: string): string => "http://localhost:3000/Box2D.simd.wasm"
//was:    `data:${new URL('../box2d-wasm', import.meta.url).toString()}/${url}  url=${url}`
})
    // gives us: http://localhost:3000/src/box2d-wasm/Box2D.simd.wasm 
