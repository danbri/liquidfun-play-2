...
//const bgdImage: HTMLImageElement = await getImage('ShimogyoCropped.png')
 const bgdImage: HTMLImageElement = await getImage('blobby.png')
...
export const onContext = ({
  ...
  const locations = getLocations({
    blob: {
      ...
      uniform: ['u_matrix', 'u_scale', 'u_radius', 'tex0'] as const // danbri
    },
    blobfullscreen: {
    ...
    fullscreen: {
      attrib: ['position'] as const,
      uniform: ['lightdir', 'tex0', 'tex1'] as const // danbri + other 'celltype' nearby too
    },
    point: {
      attrib: ['a_position', 'a_radius'] as const,
  ...
  const normalsRefractEffect = (
    ...
    //gl.uniform1i(locations.blob.uniform.celltype, 0) //danbri
...
//danbri    
    gl.uniform1i(locations.blob.uniform.celltype, 0)
  ...
  const circleHighlightColour: vec4 = vec4.fromValues(0.4, 1, 0, 1) // danbri
