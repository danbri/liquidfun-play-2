...
/* //  particleGroupDef.set_stride(0)
   // particleGroupDef.color.Set(new b2Color(1.1, 0.1, 0.1));
	let particleGroupDef = new Box2D.b2ParticleGroupDef();
    particleGroupDef.color.Set(new Box2D.b2Color(0.6, 0.6, 0.796));
    particleGroupDef.flags = 0 | Box2D.b2_colorMixingParticle;
	particleGroupDef.shape = shape;
    particleSystem.CreateParticleGroup(particleGroupDef);
	Box2D.destroy(particleGroupDef);
    shape.SetAsBox(0.4, 1.1, temp *3, 0); */
  //console.log("Flags: ", particleGroupDef.set_flags(box2d.))

  const { freeLeaked, recordLeak } = new LeakMitigator()
  const gravity = new b2Vec2(0.01, 0.05)
  ...
  temp.Set(1, 1)
  shape.SetAsBox(0.5, 0.5, temp, 0)
  particleGroupDef.shape = shape
  particleGroupDef.flags = 0 | box2D.b2_elasticParticle;
  recordLeak(particleSystem.CreateParticleGroup(particleGroupDef)) // CHANGE THE WORLD

  // NOW RE-USE these allocated objects 

  temp.Set(6, 6)
  shape.SetAsBox(0.9, 0.2, temp, 0)
  recordLeak(particleSystem.CreateParticleGroup(particleGroupDef))  // CHANGE THE WORLD

  // blob.frag sets particle color; how to pass it in from here though?
...
// danbri
  ...
  square.SetAsBox(sideLengthMetres / 16, sideLengthMetres / 16)
  const initPosition = (body: Box2D.b2Body, index: number): void => {
    temp.Set(sideLengthMetres * (Math.random() - 0.001), -sideLengthMetres * index * 0.01)
    body.SetTransform(temp, 0)
